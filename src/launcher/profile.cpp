...
//see https://json.nlohmann.me/api/adl_serializer/ and Credit to Ziv Shahaf (https://github.com/nlohmann/json/issues/1592#issuecomment-488120753)
namespace nlohmann {
	template <>
...
std::string Profile::giveLaunchCommand(const std::string& filepath, const std::string& mode) {
	...
	if (!prependAdditionalParameters.empty()) cmd << prependAdditionalParameters << " ";
...
#error "Not supported operating system"
	...
	if (this->difficultyFastMonsters) cmd << "-fast ";
	if (this->difficultyRespawnMonsters) cmd << "-respawn ";
	if (this->difficultyNoMonsters) cmd << "-nomonsters ";

	//are we JOINING a multiplayer game?
	if (mode == "join") cmd << std::format("-join {}:{} +set team {} ", this->joinAddress.ToStdString(), this->joinPort.ToStdString(), this->joinTeamNo.ToStdString());
	...
	if (this->compatflags != 0 && this->compatflags2 != 0) cmd << "+set compatmode  " << this->compatLevel << " ";
	...
	if (alwaysapplydmflags) cmd << "+set alwaysapplydmflags 1 ";
	...
	//add general flags
	if (this->enableFullscreen) cmd << "+set fullscreen 1 ";
	if (this->enableSupportWAD) cmd << "-file ./uzdoom.pk3 ";
	if (this->disableAutoload) cmd << "-noautoload ";
	...
	//load addional stuff
	if (this->enableLights) cmd << "-file ./lights.pk3 ";
	if (this->enableBrightmaps) cmd << "-file ./brightmaps.pk3 ";
	if (this->enableWidescreen) cmd << "-file ./game_widescreen_gfx.pk3 ";


	//is there something to append?
	if (!appendAdditionalParameters.empty()) cmd << appendAdditionalParameters;
