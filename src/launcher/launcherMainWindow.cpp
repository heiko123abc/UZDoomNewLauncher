...
std::string themeVar          = "system"; // system = default
...
enum
{
	...
	// Theme IDs (Must remain sequential for range binding far below)
	ID_THEME_SYSTEM,
	ID_THEME_LIGHT,
	ID_THEME_DARK
};
...
LauncherMainWindow::LauncherMainWindow(const wxString &title) : wxFrame(nullptr, wxID_ANY, title, wxDefaultPosition)
{
	...
	// Splitter for dragable description box dvider and the top panel
	wxSplitterWindow *splitter =
		new wxSplitterWindow(panel, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxSP_3D | wxSP_LIVE_UPDATE);
	...
	wxMenu *ThemeToggler = new wxMenu();
	ThemeToggler->AppendRadioItem(ID_THEME_SYSTEM, wxString::FromUTF8(GStrings.GetString("LAUNCHER_THEME_SYSTEM")));
	ThemeToggler->AppendRadioItem(ID_THEME_LIGHT, wxString::FromUTF8(GStrings.GetString("LAUNCHER_THEME_LIGHT")));
	ThemeToggler->AppendRadioItem(ID_THEME_DARK,
	                              wxString::FromUTF8(GStrings.GetString("LAUNCHER_THEME_DARK")));

	PrefMenu->AppendSubMenu(ThemeToggler, wxString::FromUTF8(GStrings.GetString("LAUNCHER_TOPBAR_PREFTHEME")));
	...
	// pre check the current theme
	int themeIdToCheck = ID_THEME_SYSTEM; // default
	if (themeVar == "system")
		themeIdToCheck = ID_THEME_SYSTEM;
	else if (themeVar == "light")
		themeIdToCheck = ID_THEME_LIGHT;
	else if (themeVar == "dark")
		themeIdToCheck = ID_THEME_DARK;
	ThemeToggler->Check(themeIdToCheck, true);
	...
	profileList = new wxDataViewListCtrl(topPanel, wxID_ANY, wxDefaultPosition, wxDefaultSize);
	...
	startGameButton =
		new wxButton(topPanel, ID_START_GAME, wxString::FromUTF8(GStrings.GetString("LAUNCHER_PROFBUTTON_START")));
	joinServerButton =
		new wxButton(topPanel, ID_JOIN_GAME, wxString::FromUTF8(GStrings.GetString("LAUNCHER_PROFBUTTON_JOIN")));
	hostServerButton =
		new wxButton(topPanel, ID_HOST_GAME, wxString::FromUTF8(GStrings.GetString("LAUNCHER_PROFBUTTON_HOST")));
	settingsButton = new wxButton(topPanel, ID_PROFILE_SETTINGS,
	                              wxString::FromUTF8(GStrings.GetString("LAUNCHER_PROFBUTTON_SETTING")));

	// move entry buttons + refresh
	refreshButton =
		new wxButton(topPanel, ID_REFRESH_LIST, wxString::FromUTF8(GStrings.GetString("LAUNCHER_PROFBUTTON_REFRESH")));
	moveEntryUpButton =
		new wxButton(topPanel, ID_MOVE_UP, wxString::FromUTF8(GStrings.GetString("LAUNCHER_PROFBUTTON_MVUP")));
	moveEntryDownButton =
		new wxButton(topPanel, ID_MOVE_DOWN, wxString::FromUTF8(GStrings.GetString("LAUNCHER_PROFBUTTON_MVDOWN")));
	...
	// the description box
	descriptionBox =
		new wxTextCtrl(splitter, wxID_ANY, wxString::FromUTF8(GStrings.GetString("LAUNCHER_NO_DSC_SELECT")),
	...
	// Theme change bindings (as above, must be in one piece)
	Bind(wxEVT_MENU, &LauncherMainWindow::OnThemeChanged, this, ID_THEME_SYSTEM, ID_THEME_DARK);
	...
	splitter->SetMinimumPaneSize(FromDIP(240)); // Prevent dragging too far
	splitter->SplitHorizontally(topPanel, descriptionBox, -150);
...
void LauncherMainWindow::OnButtonClicked(wxCommandEvent &event)
{
	...
	if (event.GetId() == ID_REFRESH_LIST)
	{
		...
		descriptionBox->ChangeValue(wxString::FromUTF8(GStrings.GetString("LAUNCHER_NO_DSC_SELECT")));
...
// also apply colors to self and recursively to spawned GUIs for custom themes
void ApplyThemeRecursive(wxWindow *window, const std::string &theme)
{
	// TODO (custom themes are hard)
}

void LauncherMainWindow::OnThemeChanged(wxCommandEvent &event)
{
	if (event.GetId() == ID_THEME_DARK)
	{
		themeVar = "dark";
	}
	else
	...
	ApplyThemeRecursive(this, themeVar);

	// tell the user to restart the launcher for application
	wxMessageBox(wxString::FromUTF8(GStrings.GetString("LAUNCHER_THEME_RESTART")), "UZDoom", wxOK | wxICON_INFORMATION);
