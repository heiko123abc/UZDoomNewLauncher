...
//Event Binder Table
wxBEGIN_EVENT_TABLE(LauncherMainWindow, wxFrame)
EVT_MENU(ID_ADD_WAD, LauncherMainWindow::OnButtonClicked)
...
LauncherMainWindow::LauncherMainWindow(const wxString& title) :wxFrame(nullptr, wxID_ANY, title, wxDefaultPosition)
{
	...
	this->SetWindowStyle(wxDEFAULT_FRAME_STYLE & ~(wxRESIZE_BORDER | wxMAXIMIZE_BOX)); //ban the user from resizing and maximizing
	...
	//the description box
	wxTextCtrl* descriptionBox = new wxTextCtrl(panel, wxID_ANY, wxT("Nothing Selected."), wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxTE_READONLY);

	//do exactly that above when event is captured and when user clicks away they will be disabled again
	profileList->Bind(wxEVT_DATAVIEW_SELECTION_CHANGED,
	...
	btnColumnSizer->AddStretchSpacer(1);//add a spacer between main buttons and move entry buttons and forces them down
...
void LauncherMainWindow::OnButtonClicked(wxCommandEvent& event)
{
	...
	if (event.GetId() == wxID_EXIT) Close(true); //close the main window
	...
	//"profileList->GetSelectedRow() != wxNOT_FOUND" blocks the user from exception when clicking profile settings after losing focus due to list refresh
	if (profileList->GetSelectedRow() != wxNOT_FOUND) {
		...
		if (event.GetId() == ID_START_GAME || event.GetId() == ID_JOIN_GAME || event.GetId() == ID_HOST_GAME) {
			...
			if (event.GetId() == ID_START_GAME) dispatchedCmd = tp.giveLaunchCommand(selectedRowPath, "").c_str();
			if (event.GetId() == ID_JOIN_GAME) dispatchedCmd = tp.giveLaunchCommand(selectedRowPath, "join").c_str();
			if (event.GetId() == ID_HOST_GAME) dispatchedCmd = tp.giveLaunchCommand(selectedRowPath, "host").c_str();
		...
		{
			//This will capture the parameters from the selected entry and pass them to the profileSettings class to prefill it

			ProfileSettings profileSettings;
			profileSettings.ProfileSettingsMenu(this, "Profile Settings", profileList->GetTextValue(profileList->GetSelectedRow(), 7).ToStdString());
