...
void attributeFromFilename(Profile *p, std::string filename) {
	...
	if (wadDatabase.contains(filename)) {
		...
		if (!info.title.empty()) p->title = info.title;
		if (!info.author.empty()) p->author = info.author;
		if (!info.releaseDate.empty()) p->releaseDate = info.releaseDate;
		if (info.isIWAD) p->isIWAD = true; //fix for wrongly detected WADS
	}
}
...
void createInitialProfile(const std::string filepath, const bool wasIWAD, const bool wasArchive) {
	...
	std::string profileFilename = "pf_" + std::filesystem::path(filepath).stem().string() + "_" + std::format("{:%Y%m%d-%H%M%S}", std::chrono::system_clock::now());
	...
	else
	{
		...
		bool success = loaderInstance.wxExtractZipFiles(zipp, wxString(path), wxTheApp->GetTopWindow());  //let wxWidget handle the extraction (dont even try to do it ourselves) - it should work on Win/Mac/Linux
			...
			for (const auto& entry : std::filesystem::directory_iterator(path)) {
				if (entry.is_regular_file()) {
					...
					if (extension == ".wad") {
						...
						wadFound = true;
						break; // Stop looking after finding the first WAD (user can fix it since there are some edgecases)
					}
				}
			}
	...
	if (inFile.is_open()) {
		try {
			...
			wxMessageBox("The launcher configuration file is corrupted and could not be read.\nPlease fix or delete the file: " + wxString(CONFIG_FILE.data()), "Error", wxOK | wxICON_ERROR);
		}
		inFile.close();
	}
	...
	//WE ARE DONE! Tell the user what is was detected at the very end as and tell them that they can change it in the profile settings later
	if (wasIWAD) {
		wxMessageBox("File was added as an IWAD.\n\nNot properly detected? It can be changed in the profile settings.", "Success on adding IWAD", wxOK | wxICON_INFORMATION);
		return;
	}
	else {
		wxMessageBox("File was added as an (P)WAD.\n\nRemember to select the correct IWAD for it before launching.\nNot properly detected? It can be changed in the profile settings.", "Success on adding (P)WAD", wxOK | wxICON_INFORMATION);
		return;
	}
}
...
bool isThisAnIWAD(const std::string filepath) {
	...
	if (memcmp(magicHeader, "IWAD", 4) == 0) { //this is nowhere near reliable but as a proof of concept it will do
		status = true;
	}
	else {
		status = false;
...
void Loader::archiveOpener(wxWindow* window) {
	...
	wxFileDialog openFileDialog(window,
...
void Loader::fileOpener(wxWindow* window){
	...
	wxFileDialog openFileDialog(window,
		"Select a Doom (P)WAD or IWAD file",
		"",
		"",
